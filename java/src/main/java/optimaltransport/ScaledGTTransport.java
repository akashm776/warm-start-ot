...
public class ScaledGTTransport {
    private int[][] CBA;
	private int[][] CAB;
	...
	// TODO - Create a method called int[]getDuals() that returns the dual weights at end (return y). 
	/**
	 * getDuals method will be called by Mapping.java after GTTransport is called.
	 * @return y - The set of dual weights at the end of the current scale.
	 */
    public int[] getDuals() {
        return adjustDualWeights();
    }
	...
	public int getNumAdjustCalls(){
		return this.numAdjustCalls;
	}
	...
	//solved once.
	public ScaledGTTransport(int[][] C, int[] supplies, int[] demands, int n, int[] duals) {
		...
		y = duals; //TODO - Set y = duals (duals is a 2n array passed through the method header) 
	...
	// Method for making sure that dual weights are ideal for scaling. Edges with flow should have 0 slack end of scale.
	public int[] adjustDualWeights() throws ArrayIndexOutOfBoundsException
	{
		numAdjustCalls++;
		// Iterate through all of the A points 
		for(int i = 0; i < n; i++)
		{
			//Iterate through all the B points
			for(int j = 0; j < n; j++)
			{
				// try{
					if( CAB[i][j] < (y[i] + y[j]) )
					{
						int slack = (y[i] + y[j]) - CAB[i][j];
						//System.out.println("y[i] = " + y[i]);
						//System.out.println("y[j] = " + y[j]);
						//System.out.println("CAB[i][j] = " + CAB[i][j]);
						if(slack > 1) {
							//System.out.println("slack = " + slack);
							//System.out.println("y[i] = " + y[i]);
							//System.out.println("y[j] = " + y[j]);
							//System.out.printf("sclaedFlow[%d][%d] = %d \n", i, j, finalCapacity[i][j]);
							//System.out.printf("CAB[%d][%d] = %d \n", i, j, CAB[i][j]);
							//System.out.println("------------------------");
							y[j] = y[j] - slack + 1;
						}
				...
				// 	System.out.println("The size of first dimension of CAB is " + CAB.length);
				// 	System.out.println("The size of second dimension of CAB is " + CAB[0].length);
